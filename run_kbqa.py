...
class KbqaProcessor(DataProcessor):
  """Processor for the CoLA data set (GLUE version)."""
  ...
  def _read_csv(self,data_dir,file_name):
    df = pd.read_csv(os.path.join(data_dir, file_name),header=None,error_bad_lines=False)
    return df
  ...
  def get_train_examples(self, data_dir):
    ...
    for row in df.iterrows():
      ...
      label = tokenization.convert_to_unicode(row[3])
